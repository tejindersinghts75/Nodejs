...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (!firstname || !lastname  || !email  || !password ) {
    ...
    return res.status(404).json(new ApiError(404,[],"All the user fields are requried"))
  }
  ...
  return res.status(201).json(new ApiResponse(200,result ,token,"user created Succesfully" ));
});
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!findUser) {
    ...
    return res.status(404).json(new ApiError(404,[],"User doesn't not exist"))
  }
  ...
  const token = jwt.sign({ _id: findUser._id, email: findUser.email },process.env.ACCESS_TOKEN_SECRET);
...
const addproducts = asyncHandler(async(req,res)=>{
  ...
  let {title,price,description,deliveryCode,productColor,productSize} = req.body;
  ...
  try {
    ...
    return res.status(201).json(new ApiResponse(201, [], "Product added successfully"));
  } catch (error) {
 ...
 export { registerUser, loginUser,addproducts,logout,deleteProduct };
